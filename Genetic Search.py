...
class Population:
    ...
    def evolve(self, env, popType, surviveBySkill=0.25, surviveByChance=0.05, mutationRate=0.05):
        ...
        sortedIndvs = [x[1] for x in sorted(scores, reverse=True)]
        ...
        mutated = random.sample(range(len(parents)), k=int(len(sortedIndvs) * mutationRate))
        for index in mutated:
            parents[index][random.randint(0, len(parents[index]) - 1)] = random.randint(0, 5)
...
if __name__ == "__main__":
    ...
    pop.checkPathFitness(environ)
    fit = pop.fitnesses
    aveFit = [sum(fit) / len(fit)]
    i = 0
    while aveFit[-1] < 0 and i < 100:
        pop.evolve(environ, 'path')
        fit = pop.fitnesses
        aveFit.append(sum(fit) / len(fit))
        i += 1

    print("path fitness", aveFit)

    # Evolve a policy
    pop = Population(200, 500)
    environ = gym.make('Taxi-v3').env

    pop.checkPolicyFitness(environ)
    fit = pop.fitnesses
    aveFit = [sum(fit) / len(fit)]
    i = 0
    while aveFit[-1] < 0 and i < 200:
        pop.evolve(environ, 'policy')
        fit = pop.fitnesses
        aveFit.append(sum(fit) / len(fit))
        i += 1

    print("policy fitness", aveFit)
